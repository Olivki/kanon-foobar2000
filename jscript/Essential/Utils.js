...
function setTrackRating(rating, track) {
	...
	PrintOut("Set rating of \'" + TitleFormat("%title% by %artist%", track) + "\' to " + (rating === "<not set>" ? "0" : rating) + ".");
}
...
function TitleFormat(title, metadb) {
	...
	var tempTitleFormat = fb.TitleFormat(title);
	var formattedTitle = tempTitleFormat.EvalWithMetadb(metadb);

	tempTitleFormat.Dispose();
