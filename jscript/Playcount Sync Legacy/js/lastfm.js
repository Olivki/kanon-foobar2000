_.mixin({
            lastfm: function () {
                this.notify_data = function (name, data) {
                    if (name === "2K3.NOTIFY.LASTFM") {
                        ...
                        _.forEach(panel.list_objects, function (item) {
                            if (item.mode === "lastfm_info" && item.lastfm_mode > 0) item.update();
                        });
                    }
                };
                ...
                this.auth = function () {
                    var api_sig = md5(
                        "api_key" + this.api_key + "methodauth.getMobileSessionpassword" + this.password + "username" +
                        this.username + this.secret);
                    var data = "format=json&password=" + this.password + "&username=" + this.username +
                               "&method=auth.getMobileSession&api_key=" + this.api_key + "&api_sig=" + api_sig;
                    ...
                    this.xmlhttp.onreadystatechange = _.bind(function () {
                        if (this.xmlhttp.readyState === 4) {
                            ...
                            if (data.error) {
                                ...
                                WshShell.popup(
                                    "Your password was verified successfully. It will not be saved anywhere.", 0,
                                    panel.name, popup.info);
                            }
                        }
                    }, this);
                };
                ...
                this.update_secret = function () {
                    this.secret = _.input("Enter your Last.fm secret", panel.name, this.secret);
                    if (this.secret.length > 0) {
                        this.write_ini("secret", this.secret);
                        window.NotifyOthers("2K3.NOTIFY.LASTFM", "update");
                        this.notify_data("2K3.NOTIFY.LASTFM", "update");
                    }
                };
        ...
        if (a <= 127) {
            ...
            if (a <= 2047) {
                ...
                if (a <= 65535) {
                    ...
                    if (a <= 2097151) {
                        b +=
                            String.fromCharCode(240 | ((a >>> 18) & 7), 128 | ((a >>> 12) & 63), 128 | ((a >>> 6) & 63),
                                                128 | (a & 63));
                    }
                }
            }
