...
function on_item_focus_change() {
    if (panel.selection < 2) panel.item_focus_change();
}
            ...
            panel: function (name, features) {
                ...
                this.colours_changed = function () {
                    if (window.InstanceType) {
                        this.colours.background = window.GetColourCUI(1);
                        this.colours.text = window.GetColourCUI(0);
                        this.colours.highlight = window.GetColourCUI(2);
                    } else {
                ...
                this.rbtn_up = function (x, y, object) {
                    ...
                    if (this.check_feature("custom_background")) {
                        this.background_menu.AppendMenuItem(MF_STRING, 100, window.InstanceType ?
                                                                            "Use default UI setting" :
                        ...
                        this.background_menu.AppendMenuItem(this.colours.mode == 2 ? MF_STRING : MF_GRAYED, 103,
                                                            "Set custom colour...");
                        this.background_menu.AppendTo(this.m, window.IsTransparent ? MF_GRAYED : MF_STRING,
                                                      "Background");
                        this.m.AppendMenuSeparator();
                    }
                    ...
                    switch (true) {
                        ...
                        case idx == 112:
                            ...
                            break;
                        case idx == 120:
                            this.artist_tf = _.input(
                                "The default is $meta(artist,0) so only the first value is used.\n\nYou can use the full foobar2000 title formatting syntax here.",
                                "Artist field remapping", this.artist_tf);
                            if (this.artist_tf == "") this.artist_tf = DEFAULT_ARTIST;
                ...
                this.tf = function (t) {
                    ...
                    if (fb.IsPlaying && (path.indexOf("http") == 0 || path.indexOf("mms") == 0)) return _.tfe(
                        t); else return _.tf(t, this.metadb);
                };
                ...
                this.artist_tf = this.check_feature("remap") ?
                                 window.GetProperty("2K3.PANEL.ARTIST.TF", DEFAULT_ARTIST) :
                                 DEFAULT_ARTIST;
                if (this.check_feature("custom_background")) {
