_.mixin({
            scrobbler: function (x, y, size) {
                this.notify_data = function (name, data) {
                    if (name == "2K3.NOTIFY.LOVE") this.post(
                        _.tf("%LASTFM_LOVED_DB%", data) == 1 ? "track.unlove" : "track.love", data);
                };
                ...
                this.playcount_sync = function () {
                    ...
                    switch (true) {
                        case !this.enabled:
                            return;
                        case this.time_elapsed == this.target_time:
                            if (!this.loved_working && !this.playcount_working) this.get("track.getInfo",
                                                                                         fb.GetNowPlaying());
                            break;
                    }
                };
                ...
                this.post = function (method, metadb) {
                    ...
                    //This is just so the correct info will be submitted to last.fm for my own media library because of
                    // the way it's setup.
                    if (!artist.contains("CV:") && artist.contains("(") && artist.endsWith(")")) {
                        if (artist.match(
                            /[\u3000-\u303f\u3040-\u309f\u30a0-\u30ff\uff00-\uff9f\u4e00-\u9faf\u3400-\u4dbf]/)) {
                            panel.console(artist);
                            var encasedName = artist.substring(0, artist.length - 1);
                    ...
                    if (!_.tagged(artist) || !_.tagged(track)) return;
                    switch (method) {
                        case "track.love":
                        case "track.unlove":
                            panel.console(
                                "Attempting to " + (method == "track.love" ? "love " : "unlove ") + _.q(track) +
                                " by " + _.q(artist));
                            panel.console("Contacting Last.fm....");
                            var api_sig = md5(
                                "api_key" + lastfm.api_key + "artist" + artist + "method" + method + "sk" + lastfm.sk +
                                "track" + track + lastfm.secret);
                            var post_data = "sk=" + lastfm.sk + "&artist=" + encodeURIComponent(artist) + "&track=" +
                                            encodeURIComponent(track);
                            break;
                        case "track.scrobble":
                            var api_sig = md5(
                                "album" + album + "api_key" + lastfm.api_key + "artist" + artist + "duration" +
                                duration + "method" + method + "sk" + lastfm.sk + "timestamp" + this.timestamp +
                                "track" + track + lastfm.secret);
                            var post_data = "format=json&sk=" + lastfm.sk + "&duration=" + duration + "&timestamp=" +
                                            this.timestamp + "&album=" + encodeURIComponent(album) + "&artist=" +
                                            encodeURIComponent(artist) + "&track=" + encodeURIComponent(track);
                            break;
                        case "track.updateNowPlaying":
                            var api_sig = md5(
                                "api_key" + lastfm.api_key + "artist" + artist + "duration" + duration + "method" +
                                method + "sk" + lastfm.sk + "track" + track + lastfm.secret);
                            var post_data = "format=json&sk=" + lastfm.sk + "&duration=" + duration + "&artist=" +
                                            encodeURIComponent(artist) + "&track=" + encodeURIComponent(track);
                            break;
                        default:
                            return;
                ...
                this.get = function (method, metadb, p) {
                    if (lastfm.api_key.length != 32) return panel.console("Last.fm API KEY not set.");
                    if (lastfm.username.length == 0) return panel.console("Last.fm Username not set.");
                    var url = lastfm.get_base_url() + "&method=" + method;
                    switch (method) {
                        case "track.getInfo":
                            if (this.loved_working || this.playcount_working) return;
                            var artist = _.tf("%artist%", metadb);
                            var track = _.tf("%title%", metadb);
                            if (!_.tagged(artist) || !_.tagged(track)) return;
                            // must use autocorrect now even when it is disabled on website
                            url +=
                                "&username=" + lastfm.username + "&artist=" + encodeURIComponent(artist) + "&track=" +
                                encodeURIComponent(track) + "&autocorrect=1&s=" + _.now();
                            break;
                        case "user.getLovedTracks":
                            if (!this.loved_working) return panel.console("Import aborted.");
                            this.page = p;
                            url += "&limit=200&user=" + lastfm.username + "&page=" + this.page;
                            break;
                        case "user.getTopTracks":
                            if (!this.playcount_working) return panel.console("Import aborted.");
                            this.page = p;
                    ...
                    switch (method) {
                        ...
                        case "track.unlove":
                            if (this.xmlhttp.responsetext.indexOf("ok") > -1) {
                                panel.console("Track " + (method == "track.love" ?
                                                          "loved successfully." :
                                                          "unloved successfully."));
                                fb.RunContextCommandWithMetadb("Customdb Love " + (method == "track.love" ? 1 : 0),
                                                               metadb, 8);
                            } else {
                        ...
                        case "track.scrobble":
                            ...
                            if (data.error) {
                                ...
                                if (data.ignored == 1) panel.console(
                                    "Track not scrobbled. The submission server refused it possibly because of incomplete tags or incorrect system time."); else if (data.accepted ==
                                                                                                                                                                     1) panel.console(
                                    "Track scrobbled successfully."); else {
                                    if (this.attempt == 1) panel.console("Unexpected submission server response.");
                                    if (this.attempt < 5) {
                                        this.attempt++;
                        ...
                        case "track.updateNowPlaying":
                            var data = _.jsonParse(this.xmlhttp.responsetext);
                            if (data.error) panel.console(data.message); else if (_.get(data,
                                                                                        "nowplaying.ignoredMessage.code") ==
                                                                                  0) panel.console(
                                "Now playing notification updated ok.");
                            break;
                        case "track.getInfo":
                            var data = _.jsonParse(this.xmlhttp.responsetext);
                            if (data.error) return panel.console(data.message);
                            if (!data.track) return panel.console("Unexpected server error.");
                            fb.RunContextCommandWithMetadb("Customdb Love " + (data.track.userloved == 1 ? 1 : 0),
                                                           metadb, 8);
                            if (fb.PlaybackLength < this.min_length) return;
                            ...
                            switch (true) {
                                case new_playcount < old_playcount:
                                    panel.console(
                                        "Playcount returned from Last.fm is lower than current value. Not updating.");
                                    break;
                                case new_playcount == old_playcount:
                        ...
                        case "user.getLovedTracks":
                            ...
                            if (data.length) {
                                _.forEach(data, function (item) {
                                    ...
                                    this.sql +=
                                        "INSERT OR REPLACE INTO quicktag(url,subsong,fieldname,value) VALUES('" + url +
                                        "','-1','LASTFM_LOVED_DB','1');\r\n";
                                    this.r++;
                                }, this);
                            ...
                            if (data.length) {
                                _.forEach(data, function (item) {
                                    ...
                                    if (playcount > 0) {
                                        ...
                                        this.sql +=
                                            "INSERT OR REPLACE INTO quicktag(url,subsong,fieldname,value) VALUES('" +
                                            url + "','-1','LASTFM_PLAYCOUNT_DB','" + playcount + "');\r\n";
                                        this.r++;
                                    }
                                }, this);
                ...
                this.update_playcount = function (metadb, new_value) {
                    ...
                    window.SetTimeout(_.bind(function () {
                        ...
                        var cmd = _.shortPath(this.sqlite3_file) + " " + _.shortPath(this.db_file) +
                                  " \"INSERT INTO quicktag(url,subsong,fieldname,value) VALUES('" + crc32 +
                                  "','-1','LASTFM_PLAYCOUNT_DB','" + new_value + "');\"";
                        //panel.console(_.shortPath(this.sqlite3_file) + " " + _.shortPath(this.db_file));
                        var attempt = 1;
                ...
                this.finish_import = function () {
                    if (this.loved_page_errors + this.playcount_page_errors > 0) {
                        panel.console("Loved track page errors: " + this.loved_page_errors +
                                      " (200 records are lost for every page that fails.)");
                        panel.console("Playcount page errors: " + this.playcount_page_errors +
                                      " (100 records are lost for every page that fails.)");
                    } else {
                        panel.console("There were no errors reported.");
                    }
                    _.run(_.shortPath(this.cmd_file), _.shortPath(this.sqlite3_file), _.shortPath(this.db_file),
                          _.shortPath(this.sql_file));
                };
                ...
                this.update_button = function () {
                    ...
                    buttons.buttons.scrobbler = new _.button(this.x, this.y, this.size, this.size, {
                        normal: n
                    }, _.bind(function () {
                        this.menu();
                    }, this), tooltip);
                ...
                this.menu = function () {
                    ...
                    m.AppendMenuItem(flag, 57, "Last.fm secret...");
                    ...
                    var idx = m.TrackPopupMenu(this.x, this.y + this.size - 10);
                    switch (idx) {
                        case 1:
                ...
                this.interval_func = _.bind(function () {
                    if (!this.loved_working && !this.playcount_working) return;
                    if (this.page != this.last_page) return this.last_page = this.page;
                    var tmp = this.page > 1 ? this.page - 1 : 1;
                    this.xmlhttp.abort();
                    if (this.loved_working) this.get("user.getLovedTracks", null,
                                                     tmp); else if (this.playcount_working) this.get(
                        "user.getTopTracks", null, tmp);
                }, this);
