_.mixin({
	scrobbler: function(x, y, size) {
		...
		this.post = function(method, metadb) {
			...
			//This is just so the correct info will be submitted to last.fm for my own media library because of the way it's setup.
			if (!artist.contains("CV:") && artist.contains("(") && artist.endsWith(")")) {
				if (artist.match(/[\u3000-\u303f\u3040-\u309f\u30a0-\u30ff\uff00-\uff9f\u4e00-\u9faf\u3400-\u4dbf]/)) {
		...
		this.update_playcount = function(metadb, new_value) {
			...
			window.SetTimeout(_.bind(function() {
				...
				//panels.Console(_.shortPath(this.sqlite3_file) + " " + _.shortPath(this.db_file));
				var attempt = 1;
