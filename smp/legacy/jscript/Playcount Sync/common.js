...
function lastfm(qs, user_agent, func) {
	...
	xmlhttp.open("GET", "http://ws.audioscrobbler.com/2.0/?format=json&api_key=" + api_key + "&username=" + username + qs + "&s=" + Math.random(), true);
...
function read_images() {
	...
	for(i = 0; i < folders.length; i++) {
		if(fso.FolderExists(folders[i])) files = files.concat(utils.Glob(folders[i] + "\\*.jpg"),utils.Glob(folders[i] + "\\*.png"),utils.Glob(folders[i] + "\\*.gif"));
	}
	return(files);
}
...
function my_metadb_changed(a, func) {
	if(fb.IsPlaying && g_metadb.RawPath.indexOf("file://") != 0) {
		if(window.GetProperty("mode") == 102) {
			if(folder == fb.TitleFormat(a).Eval()) return;
			folder = fb.TitleFormat(a).Eval();
		} else {
			if(artist == fb.TitleFormat(a).Eval()) return;
			artist = fb.TitleFormat(a).Eval();
			folder = data_folder + fb.TitleFormat("$crc32(" + a + ")").Eval();
		}
	} else {
		if(window.GetProperty("mode") == 102) {
			if(folder == fb.TitleFormat(a).EvalWithMetadb(g_metadb)) return;
			folder = fb.TitleFormat(a).EvalWithMetadb(g_metadb);
		} else {
			if(artist == fb.TitleFormat(a).EvalWithMetadb(g_metadb)) return;
			artist = fb.TitleFormat(a).EvalWithMetadb(g_metadb);
			folder = data_folder + fb.TitleFormat("$crc32(" + a + ")").EvalWithMetadb(g_metadb);
		}
	}
	func();
}
